...
at `https://main.d3qcuq1iew2ake.amplifyapp.com` (the Amplify default URL). When a hosted zone is available, bind the chosen
custom domain.
...
- Confirm the new URL serves the same content as `https://main.d3qcuq1iew2ake.amplifyapp.com` over HTTPS.
...
- Live URL today: `https://main.d3qcuq1iew2ake.amplifyapp.com`.
...
6. HTTPS-GET the new URL; confirm 200 + same content as `https://main.d3qcuq1iew2ake.amplifyapp.com`.
7. Update `.agent/current-state.md` and `.agent/c4-model.json`.

## Verification Plan

- Amplify reports `AVAILABLE` for the new domain mapping.
- `curl -sSf https://<new-domain>/` returns 200 and serves the same content as `https://main.d3qcuq1iew2ake.amplifyapp.com`.
